...
Raphael.fn.prettyReuleauxRing = function(x, y, r, ir) {
  r  = r || 1;
  ir = ir || 1;
  ...
  for (i = 0; i < line_count; i++) {
    var line = lines[i];
    ret.line = this.path("M" + line.x1 + " " + line.y1 +
                         "L" + line.x2 + " " + line.y2).attr({stroke: "#ffa9f7"});

    intersectionPoint = intersectReuleaux(line.x2, line.y2, x, y, r, x, line.y1 + r);
    this.circle(line.x2, line.y2, 2);
    this.circle(intersectionPoint.x, intersectionPoint.y, 5);
  }
...
// newx,newy: The start point of the line and center of the Reuleaux
// x,y: The end point of the line
// size: The radius of the Reuleaux.
function intersectReuleaux(newx, newy, x, y, size, fx, fy) {
...
window.onload = function() {
  ...
  var output = {};
  output.paper = Raphael("output", 200, 200);
  output.ring  = output.paper.prettyReuleauxRing(100, 100, 98, 17);
}
